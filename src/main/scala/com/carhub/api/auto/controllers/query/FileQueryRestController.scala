...
@Api(value = "File", tags = Array("Resource"), description = "This API queries the File concept.")
@RestController
...
class FileQueryRestController(@Autowired val fileQueryService : FileQueryService) {
  ...
  @ResponseBody
  def findFilesByName(@ApiParam(name = "name", value = "The filtering expression.", required = true) @PathVariable(value = "name") name : String) : ResponseEntity[_]  = {
  ...
  @ResponseBody
  def findFilesByExtension(@ApiParam(name = "format", value = "The filtering expression.", required = true) @PathVariable(value = "format") format : String) : ResponseEntity[_]  = {
