...
class ResourceQueryRestController(@Autowired val resourceQueryService : ResourceQueryService) {
                   ...
                   @ApiParam(name = "field", example="name", value = "The sort field name.", required = true) @RequestParam(name = "field") sort : String ) : ResponseEntity[_] =
  {
    ...
    if (result.isEmpty || result == null) throw new ElementNotFoundException[Resource]()
  ...
  @ResponseBody
  def countAllFiles() : Long = resourceQueryService.countAllResources()
  ...
  @ResponseBody
  def findFileByName(@ApiParam(name = "name", value = "The filtering expression.", required = true) @PathVariable(value = "name") name : String) : ResponseEntity[_]  = {
  ...
  @PreAuthorize("hasRole('READ_PRIVILEGE')")
  @GetMapping(Array("/format={format}"))
  @ResponseBody
  def findFileByExtension(@ApiParam(name = "format", value = "The filtering expression.", required = true) @PathVariable(value = "format") format : String) : ResponseEntity[_]  = {
